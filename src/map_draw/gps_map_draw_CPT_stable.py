...
from generalFunction import findOffsetLinePoint
...
def map_draw():
    ...
    x_line = np.linspace(startPointNew[0],endPointNew[0],len(GPS_all))
    y_line = np.polyval(c_line,x_line)
    GPS_all_lineOffset = np.column_stack((x_line,y_line,GPS_all_arr[:,2]))
    
    plt.figure(2)
    plt.plot(GPS_all_lineOffset[:,1],GPS_all_lineOffset[:,0],'c-')     #绘图时，将x y互换位置以匹配地图视角
    plt.plot(startPointNew[1],startPointNew[0],'ro')
    plt.plot(endPointNew[1],endPointNew[0],'bo')
    print (startPointNew[0]-start_x)*(end_x-start_x)+(startPointNew[1]-start_y)*(end_y-start_y)
    
    plt.plot(start_y,start_x,'ro')
    plt.plot(end_y,end_x,'bo')
    np.save(os.path.join(cur_path,'../../map/xueyuanLineOffset.npy'),GPS_all_lineOffset)
    #print startPointNew,endPointNew
